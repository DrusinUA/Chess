...
public class Rules : MonoBehaviour {
    ...
    void Update () 
    {
        if(dad.Action())
        {
            ...
            int fromX = GetSquareX(dad.pickPosition);
            int fromY = GetSquareY(dad.pickPosition);
            string to = GetSquare(dad.dropPosition);
           // Debug.Log("to = "+to);
          

            //Pe2e4
            string figure = chess.GetFigureAt(fromX, fromY).ToString();
    ...
    void MarkValidFigures()
    {
     ...
     void GetCoord(string e2, out int x, out int y)
    {
        x = 9;
        y = 9;
    ...
    void PlaceFigure(string box, string figure, int x, int y)
    {
        ...
        GameObject goFigure = GameObject.Find(figure); // K R k p
    ...
    void MarkSquare (int x, int y, bool isMarked)
    {
        ...
        if (isMarked)
            goCell = GameObject.Find(color + "Marked");
        else
            goCell = GameObject.Find(color);
...
class DragAndDrop
{
    ...
    void PickUp()
    {
        ...
        Vector2 clicPosition = GetClickPosition();
        Transform clickedItem = GetItemAt(clicPosition);
        if (clickedItem == null) return;

        pickPosition = clickedItem.position;
        item = clickedItem.gameObject;
        state = State.drag;
        offset = pickPosition - clicPosition;
    ...
    {
       RaycastHit2D[] figures = Physics2D.RaycastAll(position, position, 1f);
        if (figures.Length == 0)
            return null;
